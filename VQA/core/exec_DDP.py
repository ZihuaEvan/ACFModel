...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        if self.world_size > 1:
            # Initialize DDP
            net = parallel.DistributedDataParallel(net, device_ids=[self.local_rank], output_device=self.local_rank)

        net.to(self.local_rank)
        net.train()

        # Define the binary cross entropy loss
        loss_fn = torch.nn.BCELoss(reduction='sum').to(self.local_rank)

        # Define the optimizer
        optimizer = get_optim(self.__C, net, data_size)
        if self.__C.RESUME:
            ...
            ckpt = torch.load(path, map_location='cuda:{}'.format(self.local_rank))
            print('Finish!')
            net.load_state_dict(ckpt['state_dict'])

            # Load the optimizer parameters
            optimizer.optimizer.load_state_dict(ckpt['optimizer'])
            optimizer._step = int(data_size / self.__C.BATCH_SIZE * self.__C.CKPT_EPOCH)

            start_epoch = self.__C.C
