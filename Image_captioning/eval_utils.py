...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    end_index = eval_kwargs.get('word_end_ix', 0)
    ...
    while True:
        ...
        if data.get('test_labels', None) is not None and verbose_loss:
            # forward the model to get loss
            tmp = [data['fc_feats'], data['att_feats'], data['att_masks'],
                   data['train_labels'], data['test_labels'], data['train_masks'],
                   data['test_masks'], data['batch_index']]
            tmp = [_ if _ is None else _.cuda() for _ in tmp]
            fc_feats, att_feats, att_masks, train_labels, test_labels, \
            train_masks, test_masks, batch_index = tmp

            with torch.no_grad():
                pred_res = model(fc_feats, att_feats, train_labels, att_masks, seq_mask=train_masks, batch_index=batch_index)
                loss = crit(pred_res[:, 1:, :], test_labels[..., 1:], test_masks[..., 1:]).item()
        ...
        fc_feats, att_feats, att_masks, batch_index  = tmp
        with torch.no_grad():
            ...
            seq = seq.data
            entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>0).float().sum(1))
            perplexity = - seq_logprobs.gather(2, seq.unsqueeze(2)).squeeze(2).sum(1) / ((seq>0).float().sum(1))
        
        # Print beam search
        if beam_size > 1 and verbose_beam:
            for i in range(fc_feats.shape[0]):
                print('\n'.join([utils.decode_sequence(loader.get_vocab(), _['seq'].unsqueeze(0), end_index)[0] for _ in model.done_beams[i]]))
                print('--' * 10)
        sents = utils.decode_sequence(loader.get_vocab(), seq, end_index)


        for k, sent in enumerate(sents):
            entry = {'image_id': data['infos'][k]['id'], 'caption': sent, 'perplexity': perplexity[k].item(), 'entropy': entropy[k].item()}
            # entry = {'image_id': data['infos'][k]['id'], 'caption': sent, 'entropy': entropy[k].item()}
...
def eval_split_n(model, n_predictions, loader, input_data, eval_kwargs={}):
    ...
    if sample_n_method == 'bs':
        ...
        for k in range(loader.batch_size):
            _sents = utils.decode_sequence(loader.get_vocab(), torch.stack([model.done_beams[k][_]['seq'] for _ in range(sample_n)], end_index))
            for sent in _sents:
    ...
    # case 2 sample / gumbel / topk sampling/ nucleus sampling
        ...
        _sents = utils.decode_sequence(loader.get_vocab(), _seq, end_index)
        ...
        with torch.no_grad():
            model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
        for k in range(loader.batch_size):
            _sents = utils.decode_sequence(loader.get_vocab(), torch.stack([model.done_beams[k][_]['seq'] for _ in range(0, sample_n*beam_size, beam_size)]),end_index)
            for sent in _sents:
                entry = {'image_id': data['infos'][k]['id'], 'caption': sent}
                n_predictions.append(entry)
    else:
