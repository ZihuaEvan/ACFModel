...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.mask_type = getattr(opt, 'mask_type', 0)
        ...
        if 'ix_to_word' in self.info:
            ...
            self.mask_index = len(self.ix_to_word)-1
            self.end_index = len(self.ix_to_word)
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            ...
            tmp_ll = np.zeros([seq_per_img, self.seq_length + 2], dtype=np.int64)
            if hasattr(self, 'h5_label_file'):
                tmp_ll[:, 1: self.seq_length + 1] = tmp_seq
                N_sen = len(tmp_seq)
                Lm = 0
                for n in range(N_sen):
                    train_mask, test_mask, train_label, test_label, L = self.generate_mask_data(tmp_seq[n])
                    Lm = max(L, Lm)
                    if n == 0:
                        tmp_train_mask = train_mask
                        tmp_test_mask = test_mask
                        tmp_train_label = train_label
                        tmp_test_label = test_label
                    else:
                        tmp_train_mask = np.concatenate((tmp_train_mask, train_mask))
                        tmp_test_mask = np.concatenate((tmp_test_mask, test_mask))
                        tmp_train_label = np.concatenate(( tmp_train_label, train_label))
                        tmp_test_label = np.concatenate(( tmp_test_label, test_label))
                N = len(tmp_train_label)
                batch_index.append(N)

                # tmp_test_mask = tmp_test_mask[:,:Lm]
                # tmp_train_label = tmp_train_label[:,:Lm]
                # tmp_test_label = tmp_test_label[:,:Lm]
                # tmp_train_mask = tmp_train_mask[:,:Lm]
            label_batch.append(tmp_ll)
            train_label_batch.append(tmp_train_label)
            test_label_batch.append(tmp_test_label)
            train_mask_batch.append(tmp_train_mask)
            test_mask_batch.append(tmp_test_mask)
        ...
        # generate mask
        nonzeros = np.array(list(map(lambda x: (x != self.end_index).sum() + 2, data['labels'])))
        ...
        # N = len(tmp_train_label)
        # data['train_labels'] = tmp_train_label.reshape(len(batch), N, -1)
        # data['test_labels'] = tmp_test_label.reshape(len(batch), N, -1)
        # data['train_masks'] = tmp_train_mask.reshape(len(batch), N, -1)
        # data['test_masks'] = tmp_test_mask.reshape(len(batch), N, -1)
        data['train_labels'] = np.vstack(train_label_batch)
        data['test_labels'] = np.vstack(test_label_batch)
        data['train_masks'] = np.vstack(train_mask_batch)
        data['test_masks'] = np.vstack(test_mask_batch)
        data['batch_index'] = np.vstack(batch_index)
    ...
    def generate_mask_data(self, sen):
        #mask_type = 0
        new_sen = np.zeros([len(sen) + 2, ], dtype=np.int64)
        new_sen[1:len(sen) + 1] = sen
        sen = new_sen

        N = np.where(sen== 0)[0][1]
        sen[N] = self.end_index

        train_mask = np.tril(np.ones([N,len(sen)-1],dtype=np.int64))
        train_mask = np.concatenate((np.ones([N,1],dtype=np.int64),train_mask),axis=1)

        test_mask = np.zeros([N, len(sen)-1],dtype=np.int64)
        np.fill_diagonal(test_mask, 1)
        test_mask = np.concatenate((np.zeros([N, 1],dtype=np.int64), test_mask), axis=1)

        if self.mask_type >= 1:
            test_mask2 = np.triu(np.ones([N,len(sen)-1],dtype=np.int64))
            test_mask2[:,N:-1] = 0
            test_mask2 = np.concatenate((np.zeros([N, 1], dtype=np.int64), test_mask2), axis=1)

            train_mask = np.concatenate((train_mask, train_mask), axis=0)
            test_mask = np.concatenate((test_mask, test_mask2), axis=0)

        N = len(train_mask)
        gt_sen = np.tile(sen,(N,1))
        temp = np.ma.array(gt_sen, mask=test_mask)
        train_sen = temp.filled(fill_value=self.mask_index)
        return train_mask, test_mask, train_sen, gt_sen, N+1
