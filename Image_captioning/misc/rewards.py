...
def get_scores(data_gts, gen_result, opt):
    ...
    gen_result = gen_result.data.cpu().numpy()  #[batchsize *seq_per_img,seq_len+2]
    if opt.caption_model == 'ACF':# and gen_result is not None:
        for i in range(batch_size):
            try:
                end_index = np.where(gen_result[i]==opt.word_end_ix)[0][0]+1  #len of the predicted seq
